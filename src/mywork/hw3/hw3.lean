...
ANSWER HERE:
Suppose that Socrates is an object of type Person and isMortal is some function that takes Persons to Propositions. 
We take EveryoneIsMortal to be the proposition that everyone is mortal, i.e. isMortal x is true for all x a Person.
We can use this by applying EveryoneIsMortal to Socrates, and ∀ elimination gives isMortal Socrates.  
-/
...
By ∧ elimination we may conclude P. By or introduction we may conclude Q ∨ P.
-/
...
def answer : Prop := 
    ∀ (p : Person), ∃ (p1 p2 : Person), ∀ (p3 : Person), (Knows p p1) ∧ (Knows p1 p2) ∧ (Knows p2 p3)